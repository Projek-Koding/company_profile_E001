...
@section('content')
    ...
    <div class="max-w-3xl max-sm:px-6 mx-auto mb-10 px-4 pt-40 font-main">
        <h1 class="mb-10 text-center  text-4xl max-sm:text-2xl font-bold text-primary">{{ $majelis->nama_majelis }}</h1>
        ...
        <div class="mx-auto max-w-3xl rounded-lg bg-white p-6 shadow-lg ring-1 ring-gray-200">
            ...
            <div class="mb-8">
                ...
                <ul class="space-y-3 text-lg max-sm:text-sm">
                    <li><strong>Ketua:</strong> <span class="text-gray-600">{{ $ketua->nama }}</span></li>
                    @foreach ($wakil as $wakilItem)
                        <li><strong>Wakil Ketua:</strong> <span class="text-gray-600">{{ $wakilItem->nama }}</span></li>
                    @endforeach

                    @foreach ($sekretaris as $sekretarisItem)
                        <li><strong>Sekretaris:</strong> <span class="text-gray-600">{{ $sekretarisItem->nama }}</span>
                    @endforeach
            ...
            <div>
                ...
                <div class="rounded-lg bg-[#f5f8fc] p-4 shadow-inner">
                    <ul class="list-inside list-decimal space-y-2 max-sm:text-sm">
                        @foreach ($anggotaMajelis as $anggota)
                            <li class="text-gray-700">{{ $anggota->nama }}</li>
                        @endforeach
                    </ul>
                </div>
            </div>
