            ...
            <ul class="divide-y divide-gray-200">
                @foreach ($majelisList as $majelis)
                    <li class="flex items-center py-4">
                        <span
                            class="flex h-10 w-10 max-sm:w-8 max-sm:h-8 items-center justify-center rounded-full bg-primary text-white font-semibold">
                            {{ $loop->iteration }}
                        </span>
                        <a href="{{ url('/majelisDetail/' . $majelis->id) }}"
                            class="ml-4 text-lg  max-sm:text-sm font-medium text-gray-800 hover:text-[#064dac] hover:underline transition">
                            {{ $majelis->nama_majelis }}
                        </a>
                    </li>
                @endforeach
            </ul>
